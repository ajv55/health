...
export default function Day3() {
    ...
    console.log(list[2]?.[1]?.breakfast)
    ...
    const options = {
      plugins: {
          ...
          textCenter: {
            calories: list[2]?.[1]?.breakfast?.calories || "" // Access the data from props or state
        }
      }
  };
  ...
  datasets: [{
      label: 'Calories',
      data: [list[2]?.[1]?.breakfast?.calories],
      backgroundColor: '#f49541',
      borderColor: '#fefbcb',
  }]
  }

  console.log(list[2]?.[1]?.breakfast?.img)
  ...
  return (
    <div className='bg-slate-900 overflow-scroll flex flex-wrap justify-between items-center  w-full h-[32rem]'>
      <div className="flex gap-5  lg:w-[45%] w-full flex-col justify-center items-center">
        ...
        <div style={{backgroundImage: `linear-gradient(rgba(0,0,0,0.3), rgba(0,0,0,0.4)), url(${list[2]?.[1]?.breakfast?.img})`}} className="lg:w-[89%] w-[96%] h-[23rem] flex rounded-xl justify-center items-center bg-center bg-cover">
          <h1 className="text-4xl text-white font-bold tracking-wide text-center">{list[2]?.[1]?.breakfast?.meal}</h1>
        </div>
      ...
      <div className="relative lg:w-full w-[95%] lg:mr-6 mr-0 overflow-x-auto shadow-md sm:rounded-lg">
          <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
              ...
              <tbody>
                  {list[2]?.[1]?.breakfast?.ingredients.map((i: any, index: number) => {
                    return (
