...
export default function Day4() {
    ...
    console.log(list[3]?.[1]?.breakfast)
    ...
    const options = {
      plugins: {
          ...
          textCenter: {
            calories: list[3]?.[1]?.breakfast?.calories || "" // Access the data from props or state
        }
      }
  };
  ...
  datasets: [{
      label: 'Calories',
      data: [list[3]?.[1]?.breakfast?.calories],
      backgroundColor: '#f49541',
      borderColor: '#fefbcb',
  }]
  }

  console.log(list[3]?.[1]?.breakfast?.img)
    ...
    <div className='bg-slate-900 overflow-scroll flex flex-wrap justify-between items-center  w-full h-[32rem]'>
      ...
      <div className="relative w-full mr-6 overflow-x-auto shadow-md sm:rounded-lg">
          <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
              ...
              <tbody>
                  {list[3]?.[1]?.breakfast?.ingredients.map((i: any, index: number) => {
                    return (
