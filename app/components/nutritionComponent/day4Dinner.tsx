...
export default function Day4Dinner() {
    ...
    const options = {
      plugins: {
          ...
          textCenter: {
            calories: list[3]?.[1]?.dinner?.calories || "" // Access the data from props or state
        }
      }
  };
  ...
  return (
    <div className='bg-slate-900 mt-14 overflow-scroll flex flex-wrap justify-between items-center  w-full h-[32rem]'>
      <div className="flex gap-5 lg:w-[45%] w-full flex-col justify-center items-center">
        ...
        <div style={{backgroundImage: `linear-gradient(rgba(0,0,0,0.3), rgba(0,0,0,0.4)), url(${list[3]?.[1]?.dinner?.img})`}} className="lg:w-[89%] w-[96%] h-[23rem] flex rounded-xl justify-center items-center bg-center bg-cover">
          <h1 className="text-4xl text-white font-bold tracking-wide text-center">{list[3]?.[1]?.dinner?.meal}</h1>
        </div>
      ...
      <div className=" bg-gradient-to-tl from-cyan-900 via-sky-800 to-cyan-300 flex justify-center items-center p-2 lg:w-[65%] w-[96%] lg:mt-0 mt-3 h-[16rem] rounded-xl">
        <Doughnut plugins={[textCenter]} options={options} data={data} />
      </div>
      <div className="relative lg:w-full w-[96%] lg:mr-6 mr-0 overflow-x-auto shadow-md sm:rounded-lg">
          <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
              ...
              <tbody>
                  {list[3]?.[1]?.dinner?.ingredients.map((i: any, index: number) => {
                    return (
