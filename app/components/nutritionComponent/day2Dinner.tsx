...
export default function Day2Dinner() {
    ...
    const options = {
      plugins: {
          ...
          textCenter: {
            calories: list[1]?.[1]?.dinner?.calories || "" // Access the data from props or state
        }
      }
  };
  ...
  return (
    <div className='bg-slate-900 mt-14 overflow-scroll flex flex-col lg:flex-wrap lg:flex-row justify-between items-center  w-full h-[32rem]'>
      <div className="flex gap-5 lg:w-[45%] w-full flex-col justify-center items-center">
        ...
        <div style={{backgroundImage: `linear-gradient(rgba(0,0,0,0.3), rgba(0,0,0,0.4)), url(${list[1]?.[1]?.dinner?.img})`}} className="lg:w-[89%] w-[95%] h-[23rem] flex rounded-xl justify-center items-center bg-center bg-cover">
          <h1 className="text-4xl text-white font-bold tracking-wide text-center">{list[1]?.[1]?.dinner?.meal}</h1>
        </div>
      ...
      <div className="relative lg:w-full w-[90%] lg:mr-6 mr-0  overflow-x-auto shadow-md sm:rounded-lg">
          <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
              ...
              <tbody>
                  {list[1]?.[1]?.dinner?.ingredients.map((i: any, index: number) => {
                    return (
