...
export default function BreakfastLogs({name, carbs, calories, fat, protein, transFat, satFat, calcium, sodium, fiber, id}: BreakfastLogsProps) {
    ...
    const amounts = [Math.round(calories), `${protein}`, `${carbs}`, `${fat}`, `${satFat || 0}`, `${transFat === null ?  0 : transFat}`, `${fiber}`, `${sodium}`, `${calcium || 0}`];
  ...
  return (
    ...
    <div className='lg:w-[66%] w-full mr-2 flex justify-evenly items-center'>
        {amounts.map((value, index) => (
            <span key={index} className={`text-xs lg:text-xl w-16 sm:w-32 flex justify-center items-center text-indigo-400 hover:text-indigo-600 hover:cursor-pointer font-bold`}>
                {value}
            </span>
        ))}
    </div>
