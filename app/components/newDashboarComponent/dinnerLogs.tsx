...
import DinnerOptionsModal from "./dinnerOptionModal";
...
export default function DinnerLog({name, carbs, calories, fat, protein, transFat, satFat, calcium, sodium, fiber, id}: BreakfastLogsProps) {

    const dinnerLog = useSelector((state: RootState) => state.log.userDinnerLogs);
    ...
    <div className='w-full group hover:bg-indigo-300 hover:bg-opacity-45 hover:text-indigo-600 hover:cursor-pointer lg:h-14 h-16 flex lg:flex-row flex-col-reverse justify-start gap-5 items-center bg-slate-100'>
         <div className=' flex  lg:w-[32%] w-[65%] justify-between  items-center'>
            ...
            <div className="w-10 relative h-9 flex justify-center rounded-full items-center hover:bg-gray-400 hover:bg-opacity-45">
               ...
               <AnimatePresence>{isOpen && <DinnerOptionsModal onClose={() => setIsOpen(false)}  id={id!} />}</AnimatePresence>
            </div>
            
        </div>
            <div className='lg:w-[66%] w-full  mr-2   flex justify-evenly items-center'>
                {amounts.map((value, index) => (
                    <span key={index} className={`text-xs lg:text-lg w-16 sm:w-32 flex justify-center items-center text-indigo-400 hover:text-indigo-600 hover:cursor-pointer font-bold`}>{value}</span>
                ))}
            </div>
    </div>
  )
