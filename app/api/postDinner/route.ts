...
export async function POST(req: NextRequest) {
    ...
    const {
      ...
      const dinnerLog = await prisma.dinnerLog.create({
        data: {
          name,
          calories,
          fat,
      ...
      recentFoods = [dinnerLog, ...recentFoods].slice(0, limit); // Keep only the last `limit` items
    ...
    return NextResponse.json(dinnerLog, {status: 201})
}